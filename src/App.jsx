...
function RosterPanel({ roster, students, onSave }) {
  ...
  function startEditing() {
    setText(roster.map((r) => (r.name ? `${r.email}, ${r.name}` : r.email)).join("\n"));
    setEditing(true);
  }
  ...
  return (
    <div style={{ marginBottom: 22 }}>
      ...
      {editing ? (
        <div className="rl-card">
          <div className="rl-field">
            <label>One student per line: email, name (name optional)</label>
            <textarea
              value={text}
              onChange={(e) => setText(e.target.value)}
              placeholder={"jsmith@nicholsschool.org, Jamie Smith\nagarcia@nicholsschool.org, Alex Garcia"}
