...
import {
  ...
  updateLibraryBook, addCopiesToBook, deleteCopy, deleteLibraryBook,
...
function LibraryBookCard({ book, uid, communityReadCounts, reviewCountsByTitle, mySomeday, onToggleSomeday, onCheckIn, onCheckOut, onShowReviews, onEdit }) {
  const b = book;
    ...
    <div className="rl-card">
      <div className="rl-card-body">
        ...
        <div className="rl-card-text">
          ...
          {b.location && <div className="rl-isbn-status" style={{ marginTop: 3 }}>📍 {b.location}</div>}
      ...
      <div className="rl-btnrow">
        ...
        <span className="rl-card-corner">
          <button className="rl-icon-btn" title="Edit" onClick={() => onEdit(b)}><Pencil size={12} /></button>
        </span>
      </div>
    </div>
  );
...
export default function ReadingLogApp() {
  ...
  const [editLibraryBookId, setEditLibraryBookId] = useState(null); // library book id, or null
    ...
    const authUnsub = watchAuthState((u) => {
      ...
      if (u) {
        ...
        dataUnsubs = [
          ...
          subscribeRoster(CLASS_ID, setRosterState),
        ];
      } else {
        setIsTeacher(false);
      }
  ...
  async function handleUpdateLibraryBook(bookId, updates) {
    await updateLibraryBook(bookId, updates);
  }

  async function handleAddCopies(bookId, n) {
    await addCopiesToBook(bookId, CLASS_ID, n);
  }
  ...
  async function handleDeleteLibraryBook(bookId, copyIds) {
    await deleteLibraryBook(bookId, copyIds);
    setEditLibraryBookId(null);
  }

  async function handleAddLibraryBook({ title, author, genre, isbn, cover, copies: n, totalPages, location }) {
    await fsAddLibraryBook(CLASS_ID, { title, author, genre, isbn, cover, copies: n, totalPages, location });
    setLibAddOpen(false);
  }
  ...
  async function addLibraryBookQuiet({ title, author, genre, isbn, cover, copies: n, totalPages, location }) {
    await fsAddLibraryBook(CLASS_ID, { title, author, genre, isbn, cover, copies: n, totalPages, location });
  }
  ...
  async function handleSaveRoster(rows) {
    await setRoster(CLASS_ID, rows);
  }
  ...
  const myVaultEntries = vaultEntries.filter((e) => e.studentId === uid);
    ...
    <div className="rl-root">
      ...
      <div className="rl-shell">
        ...
        <nav className="rl-shelf">
          ...
          <button className={`rl-spine ${view === "myvault" ? "active" : ""}`} onClick={() => setView("myvault")}><Archive /> My Vault</button>
                ...
                <div className="rl-grid" style={{ marginBottom: 24 }}>
                  {myCheckedOut.map((b) => (
                    <LibraryBookCard
                      ...
                      onEdit={(b) => setEditLibraryBookId(b.id)}
                    />
                  ))}
                </div>
              </>
        ...
        {view === "myvault" && (
          <>
            <div className="rl-section-title">My Vault <span className="rl-count">{myVaultEntries.length}</span></div>
            {myVaultEntries.length === 0 ? (
              <div className="rl-empty">Nothing here yet. Tap "Vault" on a book you're reading to capture your first idea, question, or noticing.</div>
            ) : (
              <VaultEntryList entries={myVaultEntries} maxHeight={520} />
            )}
          </>
        )}
        ...
        {view === "teacher" && isTeacher && (
          <>
            <RosterPanel roster={roster} students={students} onSave={handleSaveRoster} />
      ...
      {editLibraryBookId && library.find((b) => b.id === editLibraryBookId) && (
        <EditLibraryBookModal
          book={library.find((b) => b.id === editLibraryBookId)}
          onClose={() => setEditLibraryBookId(null)}
          onSave={handleUpdateLibraryBook}
          onAddCopies={handleAddCopies}
          onRemoveCopy={handleRemoveCopy}
          onDelete={handleDeleteLibraryBook}
        />
      )}
...
function RosterPanel({ roster, students, onSave }) {
  const [editing, setEditing] = useState(false);
  const [text, setText] = useState("");
  const [saving, setSaving] = useState(false);

  const signedInEmails = new Set(students.map((s) => (s.email || "").toLowerCase()));
  const missing = roster.filter((r) => !signedInEmails.has((r.email || "").toLowerCase()));
  const signedInCount = roster.length - missing.length;

  function startEditing() {
    setText(roster.map((r) => [r.email, r.name || "", r.section || ""].join(", ").replace(/(, )+$/, "")).join("\n"));
    setEditing(true);
  }
  ...
  async function handleSave() {
    const rows = text
      ...
      .filter(Boolean)
      .map((line) => {
        const [email, name, section] = line.split(",").map((s) => s.trim());
        return { email: email.toLowerCase(), name: name || null, section: section || null };
      });
    setSaving(true);
    try {
      await onSave(rows);
  ...
  if (roster.length === 0 && !editing) {
    return (
      <div className="rl-card" style={{ marginBottom: 22 }}>
        <div className="rl-card-title" style={{ marginBottom: 6 }}>Set Up Your Class Roster</div>
        <div className="rl-note" style={{ marginBottom: 10 }}>
          Optional, but useful for launch day — paste your class list once and you'll see exactly who's signed in and who still needs a nudge.
        </div>
        <button className="rl-btn solid" onClick={startEditing}>Add Roster</button>
      </div>
    );
  }
  ...
  return (
    <div style={{ marginBottom: 22 }}>
      <div className="rl-section-title">
        Class Roster
        <span style={{ marginLeft: "auto" }}>
          <button className="rl-btn small" onClick={startEditing}>{roster.length === 0 ? "Add Roster" : "Edit Roster"}</button>
        </span>
      </div>
      ...
      {editing ? (
        <div className="rl-card">
          <div className="rl-field">
            <label>One student per line: email, name, section (name and section both optional)</label>
            <textarea
              value={text}
              onChange={(e) => setText(e.target.value)}
              placeholder={"jsmith@nicholsschool.org, Jamie Smith, Period 3\nagarcia@nicholsschool.org, Alex Garcia, Period 3"}
              style={{ minHeight: 160, fontFamily: "'Courier New', monospace", fontSize: 12.5 }}
            />
          </div>
          <div className="rl-btnrow">
            <button className="rl-btn" onClick={() => setEditing(false)}>Cancel</button>
            <button className="rl-btn solid" style={{ flex: 1, justifyContent: "center" }} disabled={saving || !text.trim()} onClick={handleSave}>
        ...
        <>
          <div className="rl-stat-grid">
            <div className="rl-stat"><div className="num">{signedInCount} / {roster.length}</div><div className="label">Signed In</div></div>
          </div>
          {missing.length > 0 && (
            <div className="rl-card">
              <div className="rl-isbn-status" style={{ marginBottom: 6 }}>Haven't signed in yet:</div>
              {missing.map((r) => (
                <div className="rl-checkout-row" key={r.email}>
                  <span>{r.name || r.email}{r.section ? ` — ${r.section}` : ""}</span>
                  <span className="days" style={{ color: "var(--ink-soft)" }}>{r.email}</span>
                </div>
              ))}
            </div>
          )}
...
function VaultEntryList({ entries, maxHeight = 340 }) {
      ...
      <div className="rl-modal" style={{ maxWidth: 560 }} onClick={(e) => e.stopPropagation()}>
        ...
        {vaultEntries && vaultEntries.length > 0 && (
          <>
            ...
            <VaultEntryList entries={vaultEntries} />
          </>
        )}
...
function EditLibraryBookModal({ book, onClose, onSave, onAddCopies, onRemoveCopy, onDelete }) {
  const [title, setTitle] = useState(book.title);
  const [author, setAuthor] = useState(book.author);
  const [genre, setGenre] = useState(book.genre);
  ...
  const checkedOutCount = book.copies.filter((c) => c.status === "checked_out").length;

  async function handleSave() {
    setSaving(true);
    try {
      await onSave(book.id, {
        title, author, genre, location: location || null,
        totalPages: totalPages === "" ? null : parseInt(totalPages, 10),
      });
      onClose();
    } finally {
      setSaving(false);
  ...
  async function handleDeleteClick() {
    const warning = checkedOutCount > 0
      ? `${checkedOutCount} ${checkedOutCount === 1 ? "copy is" : "copies are"} currently checked out. Delete "${book.title}" anyway? This can't be undone.`
      : `Delete "${book.title}" and all its copies? This can't be undone.`;
    if (!window.confirm(warning)) return;
    await onDelete(book.id, book.copies.map((c) => c.id));
  }
  ...
  return (
    <div className="rl-overlay" onClick={onClose}>
      <div className="rl-modal" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="rl-field"><label>Location</label><input value={location} onChange={(e) => setLocation(e.target.value)} placeholder="e.g. Bin 3, Nonfiction shelf" /></div>
        <div className="rl-field"><label>Total Pages</label><input type="number" min="1" value={totalPages} onChange={(e) => setTotalPages(e.target.value)} /></div>
        <button className="rl-btn solid" style={{ width: "100%", justifyContent: "center" }} disabled={saving || !title || !author} onClick={handleSave}>
          {saving ? "Saving..." : "Save Changes"}
        </button>

        <div className="rl-section-title" style={{ marginTop: 20 }}>Copies <span className="rl-count">{book.copies.length}</span></div>
        {book.copies.map((c) => (
          <div className="rl-checkout-row" key={c.id}>
            <span>{c.status === "checked_out" ? "Checked out" : "Available"}</span>
            {c.status === "available" ? (
              <button className="rl-icon-btn danger" title="Remove this copy" onClick={() => onRemoveCopy(c.id)}><Trash2 size={12} /></button>
            ) : (
              <span className="days" style={{ color: "var(--ink-soft)" }}>can't remove — checked out</span>
            )}
          </div>
        ))}
        <div className="rl-btnrow" style={{ marginTop: 10 }}>
          <input type="number" min="1" max="20" value={addN} onChange={(e) => setAddN(Math.max(1, parseInt(e.target.value) || 1))} style={{ width: 60, padding: "6px 9px", border: "1px solid var(--line)", borderRadius: 2 }} />
          <button className="rl-btn" onClick={() => onAddCopies(book.id, addN)}>Add {addN} Cop{addN === 1 ? "y" : "ies"}</button>
        </div>

        <div className="rl-section-title" style={{ marginTop: 20 }}>Danger Zone</div>
        <button className="rl-btn" style={{ width: "100%", justifyContent: "center", borderColor: "var(--rust)", color: "var(--rust)" }} onClick={handleDeleteClick}>
          <Trash2 size={13} /> Delete This Title
        </button>
      </div>
    </div>
  );
      ...
      <div className="rl-modal" onClick={(e) => e.stopPropagation()}>
        ...
        {mode === "single" ? (
          <>
            ...
            <div className="rl-field"><label>Location (optional)</label><input value={location} onChange={(e) => setLocation(e.target.value)} placeholder="e.g. Bin 3, Nonfiction shelf, Room 204 cart" /></div>
            <div className="rl-field">
            ...
            <button className="rl-btn solid" style={{ width: "100%", justifyContent: "center" }} disabled={!title || !author} onClick={() => onSave({ title, author, genre, isbn, cover, copies, totalPages: totalPages ? parseInt(totalPages, 10) : null, location: location || null })}>
...
function BatchAddPanel({ onAddOne, onDone }) {
  ...
  async function runBatch() {
    ...
    for (let i = 0; i < parsed.length; i++) {
      const { isbn, copies, location } = parsed[i];
      try {
        ...
        await onAddOne({
          ...
          totalPages: result.pages || null, location,
        });
    ...
    <div>
      <div className="rl-field">
        ...
        <textarea
          value={text}
          onChange={(e) => setText(e.target.value)}
          placeholder={"9780062498533\n9780786838653, 3\n9781442472426, 2, Bin 3"}
          style={{ minHeight: 130, fontFamily: "'Courier New', monospace", fontSize: 12.5 }}
        />
        <div className="rl-isbn-status">Add ", 3" after an ISBN for 3 copies (defaults to 1), and an optional third value for a shelf location — e.g. "..., 2, Bin 3".</div>
      </div>
